...
<body>
	<div class="container">
		...
		<table>
			<?php
				$sql = "SELECT DISTINCT MovieTitle FROM playsat WHERE Playing = 'false'";
				$result = mysqli_query($con, $sql);

				while ($comingsoon = mysqli_fetch_assoc($result)) {
					echo "
					<tr>
						<td><a href='movie.php?movie={$comingsoon['MovieTitle']}'>{$comingsoon['MovieTitle']}</a></td>";
					if ($comingsoon = mysqli_fetch_assoc($result)) {
						echo "<td><a href='movie.php?movie={$comingsoon['MovieTitle']}'>{$comingsoon['MovieTitle']}</a></td>";
					}
					echo "</tr>";
				}
			?>
		...
		<div class="button-wrapper space">
			<a href="now-playing.php" id="back"><button>Back</button></a>
		</div>
	</div>
</body>
</html>